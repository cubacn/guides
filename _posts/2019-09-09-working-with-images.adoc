...
关于这个主题的详细信息可参考文档：https://doc.cuba-platform.cn/manual-{cuba_version}-chs/file_storage.html[文件存储] 。
...
有几个控制上传组件行为的选项，可在这里找到： https://doc.cuba-platform.cn/manual-{cuba_version}-chs/gui_FileUploadField.html[参考文档] 。在当前案例中，使用默认行为就可以了。它允许用户上传、下载、移除和重新上传文件，并且直接将文件附加到编辑界面上的 Vet 实例。
...
参考 https://doc.cuba-platform.cn/manual-{cuba_version}-chs/gui_Image.html[CUBA 文档： 图片 UI 组件] 了解更多关于各种选项的信息。
...
TIP:  使用专门的工厂类来定义创建组件的方式只是一个对实现进行结构化的可能选项。将组件的创建提取到专门的类有几点好处，比如可以做到封装、关注点分离及在其它界面重用。 +
 +
但是也可以将组件的创建逻辑直接写在控制器中。这种方式可以在一个关于如何在UI层结构化应用程序逻辑的示例中看到。关于这个主题的详细信息可以参考本指南：https://temp.cuba-platform.cn/guides/create-business-logic-in-cuba[在 CUBA 中创建应用程序逻辑]。
...
* https://doc.cuba-platform.cn/manual-{cuba_version}-chs/gui_FileUploadField.html[CUBA 文档: 文件上传UI组件]
* https://doc.cuba-platform.cn/manual-{cuba_version}-chs/gui_Image.html[CUBA 文档：Image UI 组件]
* https://temp.cuba-platform.cn/guides/create-business-logic-in-cuba[CUBA 指南：在 CUBA 应用程序中创建业务逻辑]
